...
# command to rename a file with current date extension
class rename_to_current_date(Command):
    """
    :rename_to_current_date

    Rename the selected files to include the current date (DAY.MONTH.YEAR) format.
    """
    ...
    def execute(self):
        ...
        for fobj in self.fm.thistab.get_selection():
            dirname = os.path.dirname(fobj.path)
            basename = os.path.basename(fobj.path)
            new_basename = re.sub(r"\d{2}\.\d{2}\.\d{4}$", current_date, basename)
            if new_basename != basename:
                new_path = os.path.join(dirname, new_basename)
                os.rename(fobj.path, new_path)
                self.fm.notify(f"Renamed: {fobj.path} -> {new_path}")
#           elif '.zip' in basename:
#               # Prompt the user for a new basename
#               new_basename = input(f"Enter new name for file 'dd.mm.yyy' for {basename}:")
#               new_path = os.path.join(dirname, new_basename)
#               os.rename(fobj.path, new_path)
#               self.fm.notify(f"Renamed: {fobj.path} -> {new_path}")
            else:
                self.fm.notify(f"Skipped: {fobj.path}")

class print_selected_files(Command):
    def execute(self):
        files = [f.relative_path for f in self.fm.thistab.get_selection()]
        for f in files:
            print(f,end='\n')

# command to compress the selected file into a file named
# with current date
class zip_date(Command):
    def execute(self):
        import datetime
        ...
        # Create the zip file name based on the current date
        date_str = datetime.datetime.now().strftime("%d.%m.%Y")
        zip_name = f"{date_str}.zip"

        # Build the zip command
        command = ["zip", "-r", zip_name] + files

        # Execute the command
        self.fm.execute_command(command)
        self.fm.notify(f"Compressed to {zip_name}")


# add a duplicate command to ranger
from ranger.api.commands import Command
import os
import shutil
...
class duplicate(Command):
    def execute(self):
        ...
        # Find a new name for the duplicate file
        counter = 1
        while True:
            new_basename = "{}_copy{}".format(name, counter) + new_ext
            new_path = os.path.join(dirname, new_basename)
