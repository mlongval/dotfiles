# Generated by Powerlevel10k configuration wizard on 2026-02-12 at 19:02 EST.
...
() {
  ...
  typeset -g POWERLEVEL9K_RIGHT_PROMPT_ELEMENTS=(
    # (empty — time and toolbox moved to left prompt)
  )
  ...
  # Dynamic icon: p10k only checks ID=, not ID_LIKE=, so distros like Bluefin (ID=bluefin,
  # ID_LIKE=fedora) fall back to Tux. Read both fields and pick the right glyph.
  local _os_release='' _os_id='' _os_id_like=''
  [[ -r /etc/os-release ]] && _os_release=$(</etc/os-release)
  _os_id=${${(M)${(f)_os_release}:#ID=*}#ID=}
  _os_id_like=${${(M)${(f)_os_release}:#ID_LIKE=*}#ID_LIKE=}
  _os_id=${_os_id//\"/}; _os_id_like=${_os_id_like//\"/}
  case "${_os_id}:${_os_id_like}" in
    *bluefin*|*fedora*) typeset -g POWERLEVEL9K_OS_ICON_CONTENT_EXPANSION=$'\uF30a';;
    *ubuntu*)           typeset -g POWERLEVEL9K_OS_ICON_CONTENT_EXPANSION=$'\uF31b';;
  esac

  ################################[ prompt_char: prompt symbol ]################################
  # Transparent background.
  typeset -g POWERLEVEL9K_PROMPT_CHAR_BACKGROUND=
  # Bright yellow prompt symbol if the last command succeeded.
  # Use hex (not 256-index) so it renders correctly under tmux-direct terminfo,
  # which packs a numeric index as 24-bit RGB (e.g. 226 -> RGB(0,0,226) = blue).
